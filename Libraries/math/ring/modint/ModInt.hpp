...
private:
	...
	static ModInt crt_inner(const vector<pair<int, int>>& p) {
		ModInt ret = 0;
		size_t n = p.size();
		static int preCalcMod = -1;
...
public:
	...
	static ModInt getStirlingSecond(uint64_t n, uint64_t k) {
		...
		ModInt ret = getCombinationOnePrime(q1, q0) * seeds[r1 + 1][r0];
		if (r1 == mod() - 2 && q0 > 0) {
			ret += getCombinationOnePrime(q1, q0 - 1) * seeds[0][r0];
		}
		return ret;
	}
	static ModInt getCombinationOnePrime(uint64_t n, uint64_t k) {
	...
	static ModInt getCombinationPersistent(uint64_t n, uint64_t k) {
		if (n < k) { return 0; }
		static vector<ModInt> fact = {1}, invf = {1};
		if (n >= fact.size()) {
