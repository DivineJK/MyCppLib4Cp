//
//  Convolution_Gcd_Prod.hpp
//  MyCppLib4Cp
//
//  Created by DivineJK on 2024/01/27.
//

#ifndef Convolution_Gcd_Prod_h
#define Convolution_Gcd_Prod_h

namespace conv_gcd_internal {
...
void FZT(vector<T>* a) {
	...
	for (size_t p : primes) {
		for (size_t i = n / p; i > 0; i--) { (*a)[i - 1] += (*a)[i * p - 1]; }
	}
}
template <typename T>
void FMT(vector<T>* a) {
	...
	vector<size_t> primes = persistent_sieve(n);
	for (size_t p : primes) {
		for (size_t i = 1; i * p <= n; i++) { (*a)[i - 1] -= (*a)[i * p - 1]; }
	}
}
} // namespace conv_gcd_internal

template <typename T>
void convolve_gcd_prod(vector<T>* a, const vector<T>& b, size_t lim = UINT64_MAX) {
	...
	conv_gcd_internal::FZT(a);
	conv_gcd_internal::FZT(&y);
	for (size_t i = 0; i < n; i++) { (*a)[i] *= y[i]; }
	conv_gcd_internal::FMT(a);
}

#endif /* Convolution_Gcd_Prod_h */
